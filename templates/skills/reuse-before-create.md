...
description: Obliga a descubrir y reutilizar código, APIs nativas y patrones del repo antes de crear archivos nuevos.
---

## Objetivo

Evitar código basura y duplicación: **extender lo existente** y usar **lo nativo del runtime/framework** antes de inventar utilidades paralelas.

**Step 2 (exploración):** usar la misma mentalidad para **catalogar** en `exploration.md` (sección *Candidatos a reutilizar o extender*); no implementar aún. **Steps 5+:** aplicar antes de cada tarea de código.

## Reglas (orden obligatorio)

1. **Búsqueda semántica y por nombre** en el repo: hooks, helpers, componentes, mappers, formatters, constantes, tipos, tests similares. Si algo cubre el 80 % del caso, **adaptá o componé** en lugar de duplicar.
...
Si no encontraste nada tras buscar, indicá **qué términos buscaste** para que el revisor pueda auditar.
